...
PClassGender=subset(titanic_test,Pclass==3) 
for(i in 1:nrow(PClassGender))
{
  if(PClassGender[i,"Sex"]== "male")
  {
    PClassGender$Survived[i] = 0
  }
  else
  {
    PClassGender$Survived[i] = 1
  }
}
...
for(i in 1:nrow(titanic_test)) 
{
  ...
  {
    if(titanic_test[i,"Pclass"]=='3')
      titanic_test$Survived[i]= PClassGender$Survived[i]
    else
      titanic_test$Survived[i] = 0
  }
    if(titanic_test[i,"Embarked"]=='S')
      titanic_test$Survived[i] = 0
     else
     {
